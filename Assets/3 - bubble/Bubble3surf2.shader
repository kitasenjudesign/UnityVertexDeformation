...
Shader "custom/Bubble3surf2" {
	...
	SubShader {
        ...
        void vert(inout appdata_full v, out Input o )
        {
			...
			v.vertex.xyz = getNewVertPosition(pos,_Amount,_Time.y);
            v.normal.xyz = getNewNormal(pos,v.normal,v.tangent,_Amount,_Time.y);
		...
		void surf (Input IN, inout SurfaceOutputStandard o) {
			...
			fixed4 c = max( col,tex2D (_GrabPassTexture, IN.samplingViewportPos) );
			o.Albedo = c.rgb * 0.2;
